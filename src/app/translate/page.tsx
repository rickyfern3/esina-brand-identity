...
function Tag({ children }: { children: string }) {
  return (
    <span
      ...
      style={{ background: "rgba(255,255,255,0.1)", borderRadius: "2px", color: "rgba(255,255,255,0.7)" }}
    >
      {children}
    </span>
  );
...
function AntiTag({ children }: { children: string }) {
  return (
    <span
      className="inline-flex items-center text-xs px-2 py-0.5"
      style={{ background: "rgba(220,38,38,0.1)", borderRadius: "2px", color: "rgba(252,165,165,0.7)", border: "1px solid rgba(220,38,38,0.15)" }}
    >
      {children}
    </span>
  );
...
function BlendCard({ blend }: { blend: IdentityBlend }) {
  ...
  return (
    <div className="p-4" style={{ background: "rgba(0,0,0,0.12)", borderRadius: "2px", border: "1px solid rgba(255,255,255,0.06)" }}>
      <div className="flex items-start justify-between gap-3 mb-2">
        <span className="text-sm text-white">{blend.blend_name}</span>
        <div className="flex items-center gap-2 flex-shrink-0">
          <div className="w-14 h-px overflow-hidden" style={{ background: "rgba(255,255,255,0.08)" }}>
            <div style={{ width: `${strengthPct}%`, height: "1px", background: "rgba(255,255,255,0.6)" }} />
          </div>
          <span className="font-goldman text-xs" style={{ color: "rgba(255,255,255,0.5)" }}>{strengthPct}%</span>
        </div>
      </div>
      <div className="flex flex-wrap gap-1.5 mb-2">
        {blend.blend_dimensions.map((d) => <Tag key={d}>{d}</Tag>)}
      </div>
      <p className="text-xs leading-relaxed" style={{ color: "rgba(255,255,255,0.4)" }}>{blend.blend_description}</p>
    </div>
  );
}
...
function InputSection({ label, hint, children }: { label: string; hint?: string; children: React.ReactNode }) {
  return (
    <div>
      <label className="block text-xs mb-2" style={{ color: "rgba(255,255,255,0.55)" }}>
        {label}{hint && <span style={{ color: "rgba(255,255,255,0.3)", marginLeft: "4px" }}>{hint}</span>}
      </label>
      {children}
    </div>
  );
  ...
  return (
    <div style={{ border: "1px solid rgba(255,255,255,0.08)", borderRadius: "2px", overflow: "hidden" }}>
      <button
        ...
        style={{ background: "rgba(0,0,0,0.12)", transition: "background 0.15s" }}
      >
        <div className="flex items-center gap-2">
          <span className="text-xs" style={{ color: "rgba(255,255,255,0.65)", letterSpacing: "0.08em" }}>{title}</span>
          {badge && (
            <span className="text-[10px] px-1.5 py-0.5" style={{ background: "rgba(255,255,255,0.08)", borderRadius: "2px", color: "rgba(255,255,255,0.45)" }}>
              {badge}
            </span>
          )}
        </div>
        <svg className={`w-3.5 h-3.5 transition-transform ${open ? "rotate-180" : ""}`} fill="none" stroke="rgba(255,255,255,0.4)" viewBox="0 0 24 24">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" />
        </svg>
      </button>
      {open && <div className="p-4 space-y-4" style={{ background: "rgba(0,0,0,0.08)" }}>{children}</div>}
    </div>
  );
}
  ...
  return (
    <div className="min-h-screen">
      ...
      <header
        ...
        style={{ borderBottom: "1px solid rgba(255,255,255,0.08)", backdropFilter: "blur(12px)", background: "rgba(122,122,118,0.7)" }}
      >
        <div className="max-w-6xl mx-auto px-6 py-4 flex items-center justify-between">
          <Link href="/" className="font-goldman text-white tracking-[3px] text-sm uppercase">ESINA</Link>
      ...
      <main className="max-w-6xl mx-auto px-6 py-12">
        ...
        <div className="mb-10 flex items-start justify-between gap-6 flex-wrap fade-up-1">
          <div>
            <p className="section-tag mb-5">consumer identity translator</p>
            <h1 className="font-goldman text-4xl text-white mb-3">translate behavioral signals</h1>
          ...
          <button
            ...
            className="btn-secondary flex-shrink-0 text-xs px-4 py-2"
          >
        ...
        <div className="grid lg:grid-cols-2 gap-6">
          ...
          <div className="space-y-3 fade-up-2">
            <div className="p-6 space-y-5" style={{ background: "rgba(0,0,0,0.12)", borderRadius: "2px" }}>
              <p className="section-tag">core signals</p>
          ...
          <div className="space-y-4 fade-up-3">
            {!profile && !loading && (
              <div className="p-12 text-center" style={{ background: "rgba(0,0,0,0.08)", borderRadius: "2px" }}>
                <div className="w-10 h-10 flex items-center justify-center mx-auto mb-4" style={{ background: "rgba(0,0,0,0.12)", borderRadius: "2px" }}>
                  <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="rgba(255,255,255,0.4)" strokeWidth={1.2}>
              ...
              <>
                ...
                <div style={{ background: "rgba(0,0,0,0.12)", borderRadius: "2px" }}>
                  <div className="px-5 py-4" style={{ borderBottom: "1px solid rgba(255,255,255,0.08)" }}>
                    <p className="section-tag">translated identity profile</p>
                  </div>
                  <div className="p-5 space-y-5">
                      ...
                      <div className="space-y-2">
                        {(profile.archetypes || []).map((a) => (
                          <div key={a.archetype} className="flex items-center gap-3">
                            <div className="w-24 text-xs capitalize flex-shrink-0 flex items-center gap-1.5" style={{ color: "rgba(255,255,255,0.55)" }}>
                              {a.primary && <span className="w-1.5 h-1.5 rounded-full" style={{ background: "rgba(255,255,255,0.7)", flexShrink: 0 }} />}
                              {a.archetype}
                            </div>
                            <div className="flex-1 h-px overflow-hidden" style={{ background: "rgba(255,255,255,0.08)" }}>
                              <div className="h-full score-bar-animate" style={{ width: `${Math.round(a.weight * 100)}%`, background: a.primary ? "rgba(255,255,255,0.75)" : "rgba(255,255,255,0.35)" }} />
                            </div>
                            <span className="font-goldman text-xs w-8 text-right" style={{ color: "rgba(255,255,255,0.4)" }}>{Math.round(a.weight * 100)}%</span>
                          </div>
                        ))}
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      ...
                      <div>
                        <p className="section-tag mb-2">avoids</p>
                        <div className="flex flex-wrap gap-1.5">
                    ...
                    {/* Style + Communities */}
                    <div>
                      <p className="section-tag mb-2">aesthetic style</p>
                      <div className="flex flex-wrap gap-1.5">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      {[
                        ...
                        <div key={label} className="px-3 py-2" style={{ background: "rgba(0,0,0,0.1)", borderRadius: "2px" }}>
                          <div className="text-[10px] mb-0.5" style={{ color: "rgba(255,255,255,0.3)", letterSpacing: "0.08em" }}>{label}</div>
                          <div className="text-sm text-white capitalize">{(value || "—").replace(/_/g, " ")}</div>
                        </div>
                      ))}
                    </div>
                  </div>
                ...
                {profile.blends && profile.blends.length > 0 && (
                  <div style={{ background: "rgba(0,0,0,0.12)", borderRadius: "2px" }}>
                    <div className="px-5 py-4 flex items-center justify-between" style={{ borderBottom: "1px solid rgba(255,255,255,0.08)" }}>
                      <p className="section-tag">identity blend signatures</p>
                      <span className="text-xs" style={{ color: "rgba(255,255,255,0.25)" }}>{profile.blends.length} patterns</span>
                    </div>
                ...
                <div style={{ background: "rgba(0,0,0,0.12)", borderRadius: "2px" }}>
                  <div className="px-5 py-4 flex items-center justify-between" style={{ borderBottom: "1px solid rgba(255,255,255,0.08)" }}>
                    <p className="section-tag">matched brands</p>
                    <span className="text-xs" style={{ color: "rgba(255,255,255,0.25)" }}>{brandMatches.length} results · identity-ranked</span>
                  </div>
                  <div className="p-4 space-y-2">
                    {brandMatches.length === 0 && (
                      <p className="text-sm text-center py-4" style={{ color: "rgba(255,255,255,0.35)" }}>no matches found above threshold.</p>
                    )}
                    {brandMatches.map((match, i) => (
                      <div key={match.brandId} className="flex items-start gap-3 p-3" style={{ background: "rgba(0,0,0,0.08)", borderRadius: "2px" }}>
                        <span className="font-goldman text-xs w-5 text-right mt-0.5" style={{ color: "rgba(255,255,255,0.3)" }}>
                          {String(i + 1).padStart(2, "0")}
                        </span>
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center justify-between mb-1.5">
                            <a href={`/audit/${match.brandId}`} className="text-sm text-white">{match.brandName}</a>
                            <span className="font-goldman text-sm text-white tabular-nums">{match.score}%</span>
                          </div>
                          <div className="h-px mb-1.5 overflow-hidden" style={{ background: "rgba(255,255,255,0.08)" }}>
                            <div className="h-full score-bar-animate" style={{ width: `${match.score}%`, background: "rgba(255,255,255,0.6)" }} />
                          </div>
                          {match.snippet && (
                            <p className="text-xs leading-relaxed line-clamp-2" style={{ color: "rgba(255,255,255,0.35)" }}>{match.snippet}</p>
                          )}
                        </div>
                      </div>
                    ))}
